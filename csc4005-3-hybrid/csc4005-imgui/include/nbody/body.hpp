...
class BodyPool {
    ...
    std::vector<double> m;
    std::vector<int> acc_size_local;
    std::vector<int> start_pos_local;
...
public:
    ...
    BodyPool(size_t size, double position_range, double mass_range, int thread_num) :
            x(size), dx(size), y(size), dy(size), vx(size), vy(size), dvx(size), dvy(size), ax(size), ay(size), dax(size), day(size), m(size),  acc_size_local(thread_num), start_pos_local(thread_num) {

        int rank;
        MPI_Comm_rank(MPI_COMM_WORLD, &rank);

        // std::cout << "inbody thread_num = " << thread_num << std::endl;
    ...
    void clear_acceleration() {
        ...
        acc_size_local.assign(acc_size_local.size(), 0.0);
        start_pos_local.assign(start_pos_local.size(), 0.0);
    }
    ...
    void update_for_tick(double elapse,
        ...
        int nthrds = thread_num();

        if (thread_num() > proc_size) {
            nthrds = proc_size;
        }
        ...
        #pragma omp parallel
        {
            ...
            #pragma omp master
            {
                ...
                for (int k = 0; k < numthreads; k++) {
                    acc_size_local[k] = get_length(proc_size, numthreads, k);
                    start_pos_local[k] = acc_pos;
                    acc_pos = acc_pos + acc_size_local[k];
                }
            ...
            for (size_t i = static_cast<size_t>(start+start_pos_local[id]); i < static_cast<size_t>(start+start_pos_local[id]+acc_size_local[id]); ++i) {
        ...
        // collect
        // std::cout << "rank = " << rank << "; size = " << world_size << std::endl;
        // std::cout << "MPI_COMM_WORLD = " << MPI_COMM_WORLD << std::endl;
        // exit(0);
        // MPI_Barrier(MPI_COMM_WORLD);
        
        MPI_Reduce(&gravity, &gravity, 1, MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_WORLD);
